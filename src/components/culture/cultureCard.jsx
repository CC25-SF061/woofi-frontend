...
const CultureCard = ({ image, name, from }) => {
  return (
    <motion.div
      ...
      className="p-2 rounded shadow-lg relative group bg-[#180C18]"
    >
      <div className="relative md:w-[210px] md:h-[210px] w-[280px] h-[280px]  isolate flex flex-col justify-end overflow-hidden rounded-lg px-8 pb-8 pt-40  mx-auto group">
        <motion.img
          src={image}
          alt={name}
          className="absolute inset-0 h-full w-full object-cover transition duration-300 ease-in-out group-hover:blur-xs"
        ...
        <p className="font-quicksand z-10 mt-3 text-white opacity-0 transition duration-300 ease-in-out group-hover:opacity-100">
          {from}
        </p>
        <h3 className="font-inknut-antiqua text-white z-10 mt-2">{name}</h3>
      </div>
    </motion.div>
  );
};
