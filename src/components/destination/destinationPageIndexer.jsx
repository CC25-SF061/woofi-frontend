...
const DestinationPageIndexer = ({current, count}) => {
    ...
    const setPage = index => {
        if (index > count.current-1 || index < 0 || index == current.get) return;
    ...
    <div className="flex flex-row font-quicksand font-light text-white mb-18">
        ...
        <div className="flex flex-row gap-3 divide-x divide-[#555] text-xl">
            { Array.from({length: count.current}).map((_,i) => {
                    return (<motion.button
                        key={i}
                        className={i === current.get ? "pr-3 text-[#FFA666] text-2xl font-semibold cursor-pointer" : "pr-3 cursor-pointer"}
                        onClick={() => setPage(i)}
                    >
                        {1+i}
                    </motion.button>)
