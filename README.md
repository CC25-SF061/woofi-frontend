![Logo Woofi dengan text ](/Media/LogoText.png "Logo Woofi")

# **Woofi (Wonderful Of Indonesia)**
Platform digital yang bersifat menarik perhatian serta informatif tentang tempat wisata dan budaya lokal yang berasal dari Indonesia

## Description Proyek

### Apa yang dilakukan aplikasi Anda?
Memberikan sebuah informasi tentang budaya, tradisi, alat musik tradisional, tempat wisata yang bersifat subjektif teruntuk wisatawan lokal maupun internasional

### Mengapa Anda menggunakan teknologi yang Anda gunakan? 
Mengikuti perkembangan teknologi untuk memaksimalkan proyek ini supaya berjalan dengan baik

## Cara Menginstall dan Menjalankan Projek
1. Clone repository untuk menjalankan perintah berikut ini di terminal atau Git Bash Anda:
```
https://github.com/CC25-SF061/woofi-frontend.git
```
2. Buka Visual Studio Code atau editor kesayangan Anda dan klik "Clone Git Reposotory..." lalu tempelkan salinan anda. Tempat proyek ini di folder Anda.
3. Tunggu proses menganalisi kode oleh Visual Studio Code.
4. Buka terminal dengan cara "Ctrl+Shift+`" atau bisa juga dengan cara arahkan cursor ke panel atas lihat menu "Terminal" klik menu tersebut dan pilih "New Terminal".
5. Ketik ```npm install``` di terminal tersebut. Setelah itu tekan Enter. Tunggu beberapa saat untuk mendownload package.
6. Setelah selesai ketik lagi pada terminal tersebut ```npm run dev``` setelah itu tekan Enter. 
7. Tahan tombol Ctrl setelah itu klik "Local:" yang berwarna biru

## Visual Proyek

| Halaman | Preview | Halaman | Preview |
|---------|---------|---------|---------|
| **Home** | ![Halaman Home](/Media/HalamanHome.png "Halaman Home") | **Destination** | ![Halaman Destination](/Media/HalamanDestination.png "Halaman Destination") |
| **Culture** | ![Halaman Culture](/Media/HalamanCulture.png "Halaman Culture") | **Gallery** | ![Halaman Gallery](/Media/HalamanGallery.png "Halaman Gallery") |
| **Contact Us** | ![Halaman Contact Us](/Media/HalamanContactUs.png "Halaman Contact Us") | **Sign In** | ![Halaman Sign In](/Media/HalamanSignIn.png "Halaman Sign In") |
| **Profile** | ![Halaman Profile](/Media/HalamanProfile.png "Halaman Profile") | **Add Data** | ![Halaman Add Data](/Media/HalamanAddData.png "Halaman Add Data") |
| **Wishlist** | ![Halaman Wishlist](/Media/HalamanWishlist.png "Halaman Wishlist") | **Data Destination** | ![Halaman Data Destination](/Media/HalamanDataDestination.png "Halaman Data Destination") |
| **Dashboard** | ![Halaman Dashboard](/Media/HalamanDashboard.png "Halaman Dashboard") | **Users** | ![Halaman Users](/Media/HalamanUsers.png "Halaman Users") |
| **Post** | ![Halaman Post](/Media/HalamanPost.png "Halaman Post") | **Contact** | ![Halaman Contact](/Media/HalamanContact.png "Halaman Contact") |
| **Destination Detail** | ![Halaman Destination Detail](/Media/HalamanDestinationDetail.jpg "Halaman Destination Detail") | | |

## Library
- Google OAuth
- Redux
- TailwindCSS Vite
- axios
- dayjs
- framer-motion
- nanoid
- react-icons
- swiper

## Kredit
Kami mengucapkan terima kasih yang sebesar-besarnya kepada semua pihak yang telah berkontribusi, baik secara langsung maupun tidak langsung, dalam pengembangan proyek ini. Tanpa dukungan dan kontribusi mereka, proyek ini tidak akan mungkin terwujud seperti sekarang.

### Members
1. Annisa Fitriana(MS060D5X0565) - Machine Learning- Smk Negeri 1 Denpasar </br>
GitHub: ```https://github.com/Ananisafitriana```
2. I Gusti Ngurah Agung Mantra Suyasa(FS060D5Y0352) - Frotend/Backend Development- Smk Negeri 1 Denpasar </br>
GitHub: ```https://github.com/AgungMantra```
3. I Nyoman Kesava Mas Dananjaya Rangkan(FS060D5Y0105) - Frotend/Backend Development- Smk Negeri 1 Denpasar </br>
GitHub: ```https://github.com/kesavamas```
4. Sulih Muhammad Ragil(FS060D5Y0233) - Frotend/Backend Development- Smk Negeri 1 Denpasar </br>
GitHub: ```https://github.com/dklfh```
5. I Wayan Widhyadana Sadhu Gunawan(FS060D5Y0159) - Frotend/Backend Development- Smk Negeri 1 Denpasar </br>
GitHub: ```https://github.com/KelloVerra```